...
int countnum = 0; //สำหรับเปิด-ปิดกันสาดใน Manual mode
...
//สำหรับเปิด-ปิดกันสาดไว้ใน Manual mode
//stateser = true คือ อนุญาติให้เปิด-ปิดกันสาดได้
boolean stateser = false; 

//ไม่เคยเปิดกันสาดเลย = false เคยเปิดกันสาดเเล้ว = true
boolean stateserOA = false;

//ไม่เคยปิดกันสาดเลย = false เคยปิดกันสาดเเล้ว = true
...
void loop() { 
  ...
  if (a == 0) {
    ...
    //ปิดกันสาดของ Auto เรียบร้อย
    else {
      Serial.println("ปิดกันสาดของ Auto เรียบร้อย");
      if (stateser == true) {
        if (onetimeIN == 1 ) {
          if (countnum == 2 ) {
            doorINT = 1;
    ...
    //เวลาผ่านไป1วินาที เผื่ออยากให้ทำงานทุกๆกี่นาที
    if (millis() - previous_time > 1000) {
      previous_time = millis();
      
      // ปิดกันสาดของ Manual เรียบร้อย
      if (onetimeAU == 1 ) {
        if (t >= 28 ) {
          Serial.println("ปิดกันสาดของ Manual เรียบร้อย");
